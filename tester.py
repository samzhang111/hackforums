...
def of(name, num, suffix=""):
	...
	soups.append(bs(open("%s%d%s"%(name,i,suffix)).read()))
    return soups
...
def get_outliars_by_depth(tbd, d):
    """get_outliars_by_depth(tags_by_depth, depth)
    Returns all outliar tags by depth. List of lists.
    
    """
    outliars = get_outliars(tag_distribution(tbd[d][0]))
    out_tags = [get_tags_by_outliar(x, tbd[d][0]) for x in outliars]
    return out_tags
...
t1 = get_outliars_by_depth(tbd, i-1)
t2 = get_outliars_by_depth(tbd, i)
